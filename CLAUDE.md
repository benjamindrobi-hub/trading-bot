...
The `ALPACA_MODE` env var controls paper vs live trading. Default is `paper`.
Before placing any order, confirm mode via `bash scripts/alpaca.sh mode` and
include it in the Discord notification for any trade-placing run.
...
Use `bash scripts/alpaca.sh`, `bash scripts/perplexity.sh`, `bash scripts/discord.sh`.
