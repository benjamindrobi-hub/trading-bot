...
- Every API key is ALREADY exported as a process env var: ALPACA_MODE,
  ALPACA_API_KEY, ALPACA_SECRET_KEY, ALPACA_DATA_ENDPOINT,
...
- Verify env vars BEFORE any wrapper call:
  for v in ALPACA_MODE ALPACA_API_KEY ALPACA_SECRET_KEY PERPLEXITY_API_KEY \
            DISCORD_WEBHOOK_URL; do
    [[ -n "${!v:-}" ]] && echo "$v: set" || echo "$v: MISSING"
  done
- Confirm account mode before anything else:
  bash scripts/alpaca.sh mode
...
Date, mode (paper|live), ticker, side, shares, entry price, stop level, thesis, target, R:R.

STEP 7 — Notification: only if a trade was placed. Include mode.
  bash scripts/discord.sh "[<MODE>] <tickers, shares, fill prices, one-line why>"
